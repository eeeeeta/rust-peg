...
fn parse_rule<'input>(input: &'input str, state: &mut ParseState<'input>,
                      pos: usize) -> RuleResult<Rule> {
    {
        let start_pos = pos;
        {
            let seq_res = parse_expectflag(input, state, pos);
            match seq_res {
                Matched(pos, expected) => {
                    {
...
fn parse_expectflag<'input>(input: &'input str,
                            state: &mut ParseState<'input>, pos: usize)
 -> RuleResult<String> {
    {
        let choice_res =
            {
                let start_pos = pos;
                {
                    let seq_res = slice_eq(input, state, pos, "#[expected");
                    match seq_res {
                        Matched(pos, _) => {
                            {
                                let seq_res = parse___(input, state, pos);
                                                ...
                                                Matched(pos, _) => {
                                                    {
                                                        ...
                                                        match seq_res {
                                                            Matched(pos, _) =>
                                                            {
                                                                {
                                                                    let seq_res =
                                                                        parse_doubleQuotedString(input,
                                                                                                 state,
                                                                                                 pos);
                                                                    match seq_res
                                                                        {
